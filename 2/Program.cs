...
int VPE()
{
    ...
    while (true)
    {
        Console.Write("Ingrese la fila del producto: ");
        VProducto = int.TryParse(Console.ReadLine(), out producto);
        if (VProducto == true && producto >= 1 && producto <= resultadoFila)
        {
            return producto;
        }
        else
        {
            Console.WriteLine("Producto Ivalida, Presione una tecla para volver a intentar");
            Console.ReadKey();
            Console.Clear();
        }
    }
...
Console.WriteLine($"Ventas del producto {resultadoPE}:");

int fila = resultadoPE - 1;
